...
public class SwipeDeckAdapter extends BaseAdapter{
    ...
    public SwipeDeckAdapter(List<Friend> friends, Context context) {
        ...
        Collections.sort(friendList, new SwipeComparator());
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        Picasso.with(context).load(((Friend)getItem(position)).getImgUrl()).into(imageView);
        TextView nameTextView = (TextView) v.findViewById(R.id.swipe_name);
        nameTextView.setText(((Friend)getItem(position)).getName());
        TextView bDateTextView = (TextView) v.findViewById(R.id.swipe_card_bdate);
        if (((Friend)getItem(position)).getBirthDate() == null) {
            bDateTextView.setText("null");
        } else {
            bDateTextView.setText(((Friend)getItem(position)).getBirthDate().toString("dd.MM"));
        }
        return v;
    }

    class SwipeComparator implements Comparator<Friend> {
        @Override
